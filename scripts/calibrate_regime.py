...
class Calibrator:
    ...
    def calibrate(self, df):
        ...
        if n_trades > 0:
            ...
            print(f"  -> FOUND: Trades={n_trades} | WR={win_rate:.2%} | Return={avg_return:.5f} | BestThresh={best_thresh:.2f}")
            return model, best_thresh
        else:
            print(f"  -> NO TRADES found even with calibrated threshold {best_thresh:.2f}")
            return None, None
...
if __name__ == "__main__":
    calib = Calibrator()
    print("="*60)
    print("STRATEGY CALIBRATION SCAN (Last 3 Days)")
    print("="*60)
    ...
    for pair in PAIRS:
        ...
        print(f"  Data: {len(df_feat)} rows | AvgVol: {df_feat['volatility'].mean():.2e}")
        calib.calibrate(df_feat)
